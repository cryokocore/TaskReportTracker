...
const Admin = ({ username, setUser, user, designation, mailid }) => {
  ...
  const [otherViewForm] = Form.useForm();
  ...
  const [otherViewingTask, setOtherViewingTask] = useState(null);
  const [isOtherViewModalVisible, setIsOtherViewModalVisible] = useState(false);
  ...
  const filteredData = formattedData.filter((item) => {
    ...
    if (match && (startDateFilter || endDateFilter)) {
      ...
      const linkPosted =
        item.linkPostedDateTime && item.linkPostedDateTime !== "-"
          ...
          render: (_, record) => (
            <>
              <Button
                color="purple"
                variant="filled"
                onClick={() => handleOtherView(record, record.rowIndex)}
              >
                <EyeOutlined />
                View
              </Button>
  ...
  const handleOtherView = (record, index) => {
    const data = {
      ...record,
      rowIndex: index,
    ...
    setOtherViewingTask(data);
    otherViewForm.setFieldsValue(data);
    setIsOtherViewModalVisible(true);
  };
        ...
        <Content className="container mt-5 pt-5">
          ...
          <Col xs={24}>
            <Modal
              ...
              className="fw-bold"
              open={isOtherViewModalVisible}
              onCancel={() => setIsOtherViewModalVisible(false)}
              footer={null}
              width={1300}
            >
              <Form
